# Phase 18 — MCP-Native Gateway

> **Footprint impact.** Zero new runtime deps. MCP transport (stdio + Streamable HTTP) reuses the existing FastAPI + httpx the proxy already depends on. Tool-output compression reuses the existing transform pipeline. Tool-call cache reuses [Phase 14](14-hybrid-semantic-cache.md)'s layered cache. Guardrails reuse [Phase 15](15-native-guardrails.md). Adds ~ 8 MB to install (the MCP module itself).
>
> **Algorithm location.** New `src/lattice/mcp/` package owns the MCP protocol surface (federation, dispatch, registry, upstream transports). All compression / cache / guardrail work delegates to existing modules — no duplication. SDKs in [Phase 13](13-python-sdk-quality.md) / [Phase 17](17-typescript-sdk.md) are not involved; agents talk to `lattice mcp serve` over the MCP protocol directly.
>
...
| [src/lattice/cli/__init__.py](../../src/lattice/cli/__init__.py) (post-Phase 12 split) | Register `mcp` subcommand |
...
| [src/lattice/integrations/registry.py](../../src/lattice/integrations/registry.py) (post-Phase 12 split) | Each agent integration exposes `supports_mcp: bool` and `apply_mcp_endpoint(url)` |
| [src/lattice/cache/layers.py](../../src/lattice/cache/layers.py) | Add `ToolCallLayer` reusing the layer abstraction from Phase 14 |
...
| Tool calls are cached per (server, tool, arg-hash) | New `ToolCallLayer` plugs into the Phase 14 layered cache |
...
Reuses [Phase 15](15-native-guardrails.md) — specifically `injection.detect()` on the tool result text. Policy modes:
...
- OTel span ([Phase 16](16-otel-genai.md)): `mcp.tool_call` with attributes `mcp.server`, `mcp.tool`, `mcp.cache_hit`, `mcp.compression.ratio`, `mcp.upstream.latency_ms`
...
| Authenticating multi-tenant access to federated tools | [Phase 25](25-cloud-multitenant.md) |
