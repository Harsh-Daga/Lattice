# Phase 13 — Python SDK as a Thin Client (zero algorithm duplication)
...
> **Outcome.** The README quick-start actually works on the first try (closes a confirmed credibility hole). `py.typed` ships in the wheel. Sync wrappers work (`wrap_openai` detects sync vs async clients). Streaming responses already arrive decoded from the proxy (Phase 19's server-side streaming reverse), so the SDK has nothing to decode itself — it's a thin pass-through. Hooks compose. `wrap_anthropic` is exported at top-level. `LatticeProxyClient.chat.create()` returns typed objects instead of `dict[str, Any]`.
>
...
That was the wrong architectural call. **Reverse-pass already runs in the proxy** ([Phase 19 streaming-native compression](19-compression-intelligence.md)). Chunks arrive at the SDK already decoded. The SDK has no decoding to do.
...
If — and only if — the user runs the SDK without a proxy ([Phase 24](24-edge-wasm-core.md) in-process mode), the SDK calls into the *runtime's* implementations (or PyO3-bound `lattice-core-py`). Never reimplements them.
...
```python
"""Companion to scripts/check_sdk_no_algorithm_duplication.sh (Phase 24).
...
This test ships with Phase 13 and tightens with each subsequent phase.
...
| Topic | Phase |
|---|---|
| Streaming reverse-pass in the SDK | Doesn't exist; lives in the runtime ([Phase 19](19-compression-intelligence.md)). The SDK never decodes. |
| OpenTelemetry hook | Lives in [Phase 16](16-otel-genai.md), exposed as a built-in hook factory `from lattice.sdk.hooks_builtin import emit_otel`. |
| Sync wrappers for the Vercel AI SDK | TypeScript ecosystem; [Phase 17](17-typescript-sdk.md). |
| Embedding / batch / audio / files wrappers | Surfaces ship in [Phase 20](20-non-chat-surfaces.md); wrap helpers follow with that phase. |
