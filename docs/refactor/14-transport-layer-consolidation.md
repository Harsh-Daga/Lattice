# Phase 14 — Transport Layer Consolidation
...
After Phase 20, the architectural one-liner becomes:
...
| `src/lattice/telemetry/otel/spans.py` (Phase 22) | Adds transport-layer span attributes. |
...
Maps cleanly to HTTP 503 with `retry-after` when overflow=`reject`. Tenant priority (Phase 32 self-hosted auth) integrates: paying tenants get higher priority slots.
...
- OTel metrics ([Phase 22](19-otel-genai.md)): `lattice.transport.{rtt,queue_depth,in_flight,breaker_state,retries,timeouts}`
- Per-request response headers: `x-lattice-transport-rtt-ms`, `x-lattice-transport-attempt`, `x-lattice-transport-pool-utilization`
- Receipts ([Phase 28](28-receipts.md)): `transport: {attempts, total_rtt_ms, was_resumed, breaker_at_dispatch}`
- Bandit reward signal ([Phase 28](28-receipts.md)): retries / timeouts feed into the per-transform reward as cost-signals
...
To prove the consolidation works, here's what an adapter looks like after Phase 20:
...
| Provider-side cache control negotiation | Already handled by Phase 17's portability layer. |
