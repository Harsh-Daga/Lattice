# Phase 31 — Shared Core: Rust crate + PyO3 wheel + WASM bindings
...
The previous SDK plans (Python in [Phase 19](16-python-sdk-quality.md), TypeScript in [Phase 24](20-typescript-sdk.md)) drifted toward reimplementing the same primitives in each language. That's the disease this phase cures:
...
- Streaming reverse-pass happens **server-side** in the proxy (Phase 27's streaming-native compression already does this). Chunks arrive at the SDK already-decoded.
...
The TS SDK ([Phase 24](20-typescript-sdk.md)) consumes only this binding:
...
These are the data structures shared across all bindings. Critical for the cache-key compatibility invariant (Phase 20).

### Step 4 — Port streaming `ChunkBuffer`

Required for both Phase 27 (server-side streaming compression) and Phase 24 (TS in-process mode).
...
### Step 7 — Wire Phase 24 TS SDK to consume `@lattice/core-wasm` for in-process mode
...
Once Phase 24's TS SDK exists, turn on `check_sdk_no_algorithm_duplication.sh`. From this point, drift = build break.
