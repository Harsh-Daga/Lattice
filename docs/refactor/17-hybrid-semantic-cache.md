# Phase 17 — Hybrid Semantic Cache (lightweight defaults)
...
>
> **Algorithm location.** Cache layer abstractions and fingerprinting live in `src/lattice/cache/`. The IR canonical fingerprint is shared with [Phase 31](31-edge-wasm-core.md)'s shared core — both Python fallback and WASM/PyO3 binding compute byte-identical fingerprints. Cache backends (memory / Redis / Postgres) are pluggable; default is in-memory with `cachetools`.
>
...
This change has another nice property: the cached embeddings themselves are **cached** via Phase 31's per-text embedding cache, so the same query in a later request doesn't pay even the cheap embedding cost twice.
...
**No cross-tenant matching.** Every tier's keys are namespaced. Tenant comes from the request's `Authorization` header ([Phase 32 self-hosted auth](32-cloud-multitenant.md)) or `x-lattice-tenant` header or the configured default.
...
| [src/lattice/cache/fingerprint.py](../../src/lattice/cache/fingerprint.py) (Phase 16) | Add `ir_canonical_fingerprint(ir, namespace, model)`; delegates to native core when present |
| [src/lattice/cache/stores.py](../../src/lattice/cache/stores.py) (Phase 16) | Add `CASStore` mediator |
...
None — purely additive on top of Phase 16's split.
...
The Phase 31 shared core implementation must produce **byte-identical** output. Verified by parity test.
...
class UserProviderEmbeddingBackend:
    ...
    Uses Phase 31's /v1/embeddings endpoint internally with per-text caching,
...
The `dispatcher` is the same Phase 31 dispatcher that handles `/v1/embeddings` user requests — same dedup, same per-text cache. The lookup-side embedding cost converges to near-zero on steady-state workloads.
...
def resolve_tenant(request: Request, config: CacheConfig) -> str:
    ...
    if (key := _extract_bearer(request.headers)) is not None:
        # In Phase 32 self-hosted-auth this becomes a real lookup.
...
8. Footprint test passes — default install adds < 1 MB beyond Phase 16 baseline.
...
| Topic | Phase |
|---|---|
| Cross-provider cache portability (user-initiated provider switch inherits warmth) | [Phase 17](27-cache-portability.md) |
| Per-tenant distilled embedding model | **Cut from plan** — required cloud + GPU + training infra; bandit (Phase 28) covers the value with zero infra |
| Auth-driven tenant resolution | [Phase 32 self-hosted auth](32-cloud-multitenant.md) |
| Compression-aware embedding (embed the compressed text, not the raw) | [Phase 27](22-compression-intelligence.md) |
