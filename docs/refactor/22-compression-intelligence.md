# Phase 22 — Compression Intelligence: Streaming, Tool-Diff, JSON Repair (lightweight) + LLMLingua-2 (opt-in heavy)
...
>
> **Algorithm location.** Streaming chunk buffer ports to [Phase 31](31-edge-wasm-core.md)'s shared core so both Python proxy and TS/WASM edge SDK use the same implementation. Tool-diff detector is a new transform in `src/lattice/transforms/`. JSON repair patterns live in `src/lattice/safety/output/repair.py` (added in [Phase 21](18-native-guardrails.md); extended here). LLMLingua-2 lives in `src/lattice/transforms/llmlingua/` as an isolated package, never imported unless enabled.
>
...
- **LLMLingua** runs only when opted in **and** `expected_utility` beats the deterministic branch on the same segment ([Phase 28](23-segment-aware-planning.md) distortion budget).
...
3. **JSON structural repair** — pure-Python pattern matching (extends Phase 21's repair). Zero deps.
...
src/lattice/pipeline/streaming/chunk_buffer.py        # delegates to Phase 31 native core when present
...
src/lattice/safety/output/repair_v2.py                # extends Phase 21 repair with more patterns
...
class ChunkBuffer:
    ...
    Delegates to native Rust core (Phase 31) when lattice-core-py is installed —
...
90%+ token savings on the earlier result with zero quality loss (the agent has the up-to-date later result). The receipt machinery ([Phase 28](28-receipts.md)) records the diff so reconstruction is possible.
...
### 3.3 JSON structural repair (extends Phase 21)

`src/lattice/safety/output/repair_v2.py` extends [Phase 21](18-native-guardrails.md)'s repair with:
...
7. **Truncation recovery** — `finish_reason: length` → close open structures + surface `repair.truncated: true` flag for [Phase 34](26-agent-memory.md)'s continuation logic
8. **Schema-coerced field types** — `"42"` → `42` when schema demands integer

Each pattern is a pure function `(repaired, applied: bool)`. Repair runs patterns until JSON parses or no pattern made progress. **Never calls the LLM** — that's the explicit retry path in [Phase 34](26-agent-memory.md).
...
| Footprint default | `tests/integration/footprint/test_4gb_laptop.py` | ≤ Phase 20 footprint (LLMLingua not loaded) |
...
| Topic | Phase / future |
|---|---|
| Per-tenant fine-tuned LLMLingua model | **Cut from plan** — required cloud + GPU + training infra. Bandit (Phase 28) covers the value with zero infra. |
| Streaming compression on the request side | Future — most request bodies are small enough to compress whole. |
| LLM-judge in-line validation | Existing post-transform-guard (Phase 16) handles structural validation; LLM-judge is benchmark-only. |
