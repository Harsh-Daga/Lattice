...
>
> **Reading guide.** Each row reads: *"There is exactly one X. It lives in Y. Every other surface that needs X imports it from Y or consumes it across the FFI boundary in [Phase 24](24-edge-wasm-core.md). If you find yourself writing a second implementation of X, stop."*
...
| `ExecutionPlan` | `src/lattice/planner/execution_plan.py` | **Phase 12** collapses the prior duplicate in `ir/primitives/`. |
| `Candidate` + `CandidateScorer` | `src/lattice/planner/candidate.py` | **Phase 12** collapses the duplicate scoring formula. |
| `AliasTable` (PII + reference_sub merged) | `src/lattice/transforms/alias_table.py` | **Phase 15** merges PII alias entries into the existing alias table; never two parallel tables. |

## 2. Fingerprints, hashing, canonical serialization

| Primitive | Canonical home | Notes / phase |
|---|---|---|
| `xxh3_128_hexdigest` wrapper | `src/lattice/util/hashing.py` | Pre-existing-or-new in Phase 12. Wraps `xxhash` (Python) and routes to native via Phase 24's `lattice_core_py.xxh3` when present. |
...
| Provider-invariant fingerprint | same file, `provider_invariant_fingerprint` | **Phase 22**. |
| Request SHA-256 (Tier 1 cache) | `src/lattice/cache/layers/exact.py::exact_request_hash` | **Phase 14**. |
| Jaccard signature | `src/lattice/cache/layers/jaccard.py::jaccard_signature` | **Phase 14** (lifted from current `cache/semantic.py`). |
| Tenant namespace resolution | `src/lattice/cache/namespace.py::resolve_tenant` | **Phase 14**. |
| Receipt JTI / signature | `src/lattice/audit/receipts.py` | **Phase 23**. Reuses `util/hashing.py`. |
| Bearer-key hash | `src/lattice/keys/manager.py::_hash_bearer` | **Phase 25**. SHA-256; never collides. |

## 3. Transforms

| Transform | Canonical home | Notes |
|---|---|---|
| `reference_sub` | `src/lattice/transforms/reference_sub.py` | Rust acceleration via Phase 24. |
...
| `output_cleanup` | `src/lattice/transforms/output_cleanup.py` | Rust acceleration via Phase 24. |
| `rate_distortion` | `src/lattice/transforms/rate_distortion.py` | |
| `delta_encode` | `src/lattice/transforms/delta_encode.py` | |
| `tool_diff` | `src/lattice/transforms/tool_diff/` | **Phase 19**. |
| `llmlingua` | `src/lattice/transforms/llmlingua/` | **Phase 19**. Opt-in `[llmlingua]` extra. |
| `embedding_dedup` | `src/lattice/transforms/embedding_dedup.py` | **Phase 20**. |
| Transform registry | `src/lattice/transforms/registry.py` | One source. `pipeline/runner.py` factory generated from it (Phase 12). |
...
| Streaming chunk buffer | `src/lattice/pipeline/streaming/chunk_buffer.py` | **Phase 19**. Rust mirror in `crates/lattice-core/src/streaming/`. TS edge consumes via WASM only. |
...
| Beam-search representation optimizer | `src/lattice/pipeline/representation_optimizer.py` | One implementation; Phase 19 extends with LLMLingua branch. |
| Thompson bandit | `src/lattice/planner/bandit/thompson.py` | **Phase 23**. Pure numpy. |
| Profile registry | `src/lattice/policy/profiles.py` | **Phase 23**. |
| Step classifier (agent loops) | `src/lattice/agent/step_classifier.py` | **Phase 26**. |
| Step profile overrides | `src/lattice/agent/step_profiles.py` | **Phase 26**. Layers on top of `policy/profiles.py`. |
| Risk score | `src/lattice/safety/risk_scoring.py` | Pre-existing. |
| Post-transform guard (was MILV) | `src/lattice/safety/post_transform_guard.py` | **Phase 12** rename. |
| PII detector (rule, default) | `src/lattice/safety/pii/rule_detector.py` | **Phase 15**. |
| PII tokenizer (reversible via Alias Table) | `src/lattice/safety/pii/tokenizer.py` | **Phase 15**. Uses §1 `AliasTable`. |
| Injection detector (heuristic, default) | `src/lattice/safety/injection/heuristic_detector.py` | **Phase 15**. |
| Output validator + repair | `src/lattice/safety/output/validator.py` + `repair_v2.py` | **Phase 15 + Phase 19**. |
...
| Portability manifest | `src/lattice/cache/portability/manifest.py` | **Phase 22**. |
| Cache warmer | `src/lattice/cache/warmer.py` | **Phase 22**. |
| Cache analyzer | `src/lattice/cache/analyzer.py` | **Phase 22**. |
| Embeddings cache (per-text) | `src/lattice/cache/embeddings_cache.py` | **Phase 20**. Reused by Phase 14 embedding tier. |
...
| Relevance scorer (rule, default) | `src/lattice/agent/relevance.py::RuleRelevanceScorer` | **Phase 21**. |
| Relevance scorer (embedding, opt-in) | same file, `EmbeddingRelevanceScorer` | **Phase 21**. Reuses §6 backend. |
| Context GC | `src/lattice/agent/context_gc.py` | **Phase 21**. |
| Summarizer | `src/lattice/agent/summarizer.py` | **Phase 21**. Uses §7 dispatcher. |
| Token budget allocator | `src/lattice/agent/budget.py` | **Phase 21**. |
| Inference-aware retry strategy | `src/lattice/agent/retry.py` | **Phase 21**. **Disjoint from §7 transport retry** — agent-retry decides *whether* to retry (refusal / truncation / repair); transport retry decides *how* (backoff, breaker). |
| Loop state | `src/lattice/agent/loop_state.py` | **Phase 26**. |

## 7. Transport layer (Phase 27 establishes the canonical set)

| Primitive | Canonical home | Notes |
|---|---|---|
| Unified provider dispatcher | `src/lattice/transport/dispatcher.py` | **Phase 27**. Replaces per-adapter dispatch. |
| Connection pool (per provider) | `src/lattice/transport/pool.py` | **Phase 27**. HTTP/2 multiplexed when supported. |
| Retry policy | `src/lattice/transport/retry.py` | **Phase 27**. **Single implementation**. Adapters declare per-error-class policy; the runner is one file. |
| Circuit breaker | `src/lattice/transport/circuit_breaker.py` | **Phase 27**. |
| Timeout policy | `src/lattice/transport/timeout.py` | **Phase 27**. Per (operation, model-family) defaults; per-request override via header. |
| Backpressure / queue | `src/lattice/transport/backpressure.py` | **Phase 27**. Bounded queue with explicit overflow strategy. |
| Stream resumption | `src/lattice/transport/stream_resume.py` | **Phase 27**. |
| Transport metrics | `src/lattice/transport/metrics.py` | **Phase 27**. RTT / queue depth / breaker state / in-flight. Feeds §10 OTel + §11 receipts. |
| TACC congestion control | `src/lattice/transport/tacc.py` | Pre-existing; consolidated under transport in Phase 27. |
| Binary framing (LATT) | `src/lattice/protocol/framing.py` (Python fallback) + `crates/lattice-core/src/framing/` (canonical Rust) | Phase 24 Rust port. |
...
| Cost estimator | `src/lattice/telemetry/cost_estimator.py` | **One implementation.** Phase 23 receipts + Phase 25 quotas both consume from here; no second estimator anywhere. |
...
| Primitive | Canonical home | Notes |
|---|---|---|
| `Session` | `src/lattice/state/session.py` | Pre-existing. Phase 21 adds `memory_policy`; Phase 26 adds `loop_state`. |
...
| OTel exporter | `src/lattice/telemetry/otel/` | **Phase 16**. Opt-in. |
...
| Primitive | Canonical home | Notes |
|---|---|---|
| Receipts | `src/lattice/audit/receipts.py` + `receipt_store.py` + `receipt_router.py` | **Phase 23**. HMAC-signed; never contain user content. |
| AuthContext | `src/lattice/auth/context.py` | **Phase 25**. Opt-in. |
| Auth backends | `src/lattice/auth/backends/{sqlite,postgres,jwt_external}.py` | **Phase 25**. Default: SQLite. |
| Virtual-key manager | `src/lattice/keys/manager.py` | **Phase 25**. |
| Key encryption (Fernet) | `src/lattice/keys/encryption.py` | **Phase 25**. |
| Quota limiter | `src/lattice/quotas/limiter.py` | **Phase 25**. |
| Tenant policy | `src/lattice/tenants/policy.py` | **Phase 25**. |
...
| OpenAI-compat handler | `src/lattice/gateway/compat/openai_chat.py` | Phase 12 split. |
| Anthropic-compat handler | `src/lattice/gateway/compat/anthropic_messages.py` | Phase 12 split. |
| Embeddings endpoint | `src/lattice/gateway/embeddings.py` | **Phase 20**. |
| Batch endpoint | `src/lattice/gateway/batch.py` | **Phase 20**. |
| Audio endpoints | `src/lattice/gateway/audio.py` | **Phase 20**. |
| Files endpoint | `src/lattice/gateway/files.py` | **Phase 20**. |
| Realtime endpoint | `src/lattice/gateway/realtime.py` | **Phase 20**. |
| MCP server | `src/lattice/mcp/server.py` | **Phase 18**. |
| MCP dispatcher | `src/lattice/mcp/dispatcher.py` | **Phase 18**. |
...
| OpenAPI source of truth | `openapi/lattice-proxy.yaml` | **Phase 17**. Both Python TypedDicts and TS types are generated from this. |
...
| `lattice cache {warm,analyze}` | `src/lattice/cli/cache.py` | **Phase 22**. |
| `lattice {user,key,quota,usage}` | `src/lattice/auth/admin_cli.py` | **Phase 25**. |
| `lattice mcp {serve,add,list,remove}` | `src/lattice/mcp/cli.py` | **Phase 18**. |
...
| Hot reload | `src/lattice/config/reload.py` | **Phase 23**. Atomic. |
...
| Multiple bandit reward functions | Phase 23 bandit and Phase 26 step-classifier compete | §4 names one bandit; Phase 26's step classifier feeds the SAME bandit, never a parallel one |
