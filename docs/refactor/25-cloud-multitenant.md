# Phase 25 — Optional Self-Hosted Auth, Virtual Keys, Quotas
...
>
> **Footprint impact.** Default install: zero new deps, all auth features off. With `[auth]` extra: +argon2-cffi (~500 KB), +pyjwt (~100 KB), ~ 5 MB RSS. With Postgres backend (already an `[postgres]` extra from Phase 14): no additional deps. SQLite-backed state is the default when auth is enabled — no external service required.
>
> **Algorithm location.** New `src/lattice/auth/`, `src/lattice/keys/`, `src/lattice/quotas/`, `src/lattice/tenants/` modules. All reuse Phase 14's tenant namespace plumbing, [Phase 23](23-receipts-bandit-profiles.md)'s receipts, and [Phase 16](16-otel-genai.md)'s OTel spans. SDKs don't reimplement any auth logic — they pass a bearer token, the proxy does the rest.
>
...
> - Audit who-did-what via signed receipts ([Phase 23](23-receipts-bandit-profiles.md))
...
- No distillation training pipeline (Phase 23's bandit covers the value with zero infra)
- No hosted playground (the `lattice` CLI ships a local-only debug page in [Phase 26](26-agent-loop-aware.md), gated behind `--debug-ui` and bound to 127.0.0.1)
...
| Per-tenant LoRA distillation training | Requires GPU + cloud infra; bandit ([Phase 23](23-receipts-bandit-profiles.md)) covers 80% of value with zero infra |
| Hosted playground | Local debug UI in [Phase 26](26-agent-loop-aware.md); not internet-facing |
...
class TenantPolicy:
    cache: CacheConfig | None = None              # overrides for Phase 14
    guardrails: GuardrailConfig | None = None     # overrides for Phase 15
    agent_memory: AgentMemoryConfig | None = None # overrides for Phase 21
    quotas: tuple[Quota, ...] = ()
    profile: str | None = None                    # named profile from Phase 23
...
`/lattice/admin/{users,keys,quotas,usage}` — JSON REST. Requires the bearer to belong to a user with `role = admin`. Documented in OpenAPI ([Phase 17](17-typescript-sdk.md)) so the same generated TS / Python types apply.
...
Every request that touches an authenticated context produces a receipt ([Phase 23](23-receipts-bandit-profiles.md)) that records:
...
| Stripe billing | No SaaS |
| Cloudflare Workers / Neon / Upstash Terraform | Open-source self-hosted only |
| Per-tenant LoRA distillation training | Requires GPU + cloud infra; bandit ([Phase 23](23-receipts-bandit-profiles.md)) covers value with zero infra |
| Per-tenant ML personalization | Same — cut for the same reason |
| Hosted playground | Local `--debug-ui` in [Phase 26](26-agent-loop-aware.md) instead |
...
| Kubernetes operator | Out of scope — Docker image runs anywhere; Helm not packaged ([Phase 26](26-agent-loop-aware.md)) |
