...
> **Purpose.** Capture the external architecture critique (May 2026) against **what is actually on `main` after Phases 0–10**, and record what we **accept**, **reject**, and **map** into Phases 12–27. This doc prevents a parallel “refound the repo” rewrite that would undo the v1.0 refactor.
>
...
| **Orchestration gaps** | `features not reached by pipeline` (e.g. `format_conversion` on some tiers) | Fix **planner activation**, not add transforms — see [19.5-segment-aware-planning.md](19.5-segment-aware-planning.md) |
...
| **Representation optimizer** (beam search, `CandidateSearch`) | **Shipped.** `pipeline/representation_optimizer.py`, immutable `Candidate` in `ir/primitives.py` | **Extend** scoring utility — [12-honesty-pass.md](12-honesty-pass.md), [22-cache-portability.md](22-cache-portability.md), [27-transport-layer-consolidation.md](27-transport-layer-consolidation.md) |
| **Semantic transport runtime** (delta, manifest, cache planner, prefix) | **Partially shipped.** `transport/`, `protocol/`, `cache/`, Phase 22/27 docs | **Complete** via Phase 27 + 22; expose gains in planner utility |
...
| Insight | Phase | Doc |
|---|---|---|
| Constitutional lifecycles + honest optimization objective | Runtime doc + Phase 12 | [`runtime.md`](../architecture/runtime.md), [12-honesty-pass.md](12-honesty-pass.md) |
| Centralize validation entrypoints | Phase 12 | `runtime/validation_engine.py` (facade over MILV + guardrails + `ir/validation`) |
| Segment-aware planning (per-region transform policy) | **Phase 19.5** (new) | [19.5-segment-aware-planning.md](19.5-segment-aware-planning.md) |
| Utility-driven `Candidate.score` (not compression-only) | Phase 12 + 19 + 22 + 27 | `ir/quality.py`, planner inputs |
| `provider_cache_probability` in search | Phase 22 | `cache/analyzer.py` → planner |
| `transport_gain` in search | Phase 27 | delta_wire + stable prefix metrics → planner |
| Replay determinism contract | Phase 12 | `tests/contract/test_replay_determinism.py` |
| Transport-first consolidation | Phase 27 | unchanged — still M3 anchor |
| KV-cache / portability | Phase 22 | unchanged |
...
| Stop reporting compression ratio | Keep as **one term** in utility; remove as sole headline in Phase 12 README/CLAIMS alignment |
| Add random transforms | Code budget + SSOT forbid; bandit (Phase 23) adapts **existing** transforms |
...
Replace single-axis “maximize compression_ratio” with a documented utility function used by beam search and bandit (Phase 23):

```
utility(c) =
    + w_q * semantic_quality(c)
    + w_cache * provider_cache_probability(c)   # Phase 22 analyzer
    + w_tx * transport_gain(c)                 # Phase 27 (delta, prefix stability)
    - w_tok * normalized_token_cost(c)
    - w_lat * latency_p95_estimate(c)
    - w_inst * instability_penalty(c)           # replay / placeholder leakage
```

Weights are **profile-configurable** ([Phase 23](23-receipts-bandit-profiles.md)), not hard-coded magic numbers. Default profile favors **reasoning preservation** on agent workloads.
...
| `cache` tiers + portability | Provider KV | `cache/`, Phase 14/22 |
