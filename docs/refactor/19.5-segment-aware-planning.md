# Phase 19.5 — Segment-Aware Planning
...
>
> **Transport role.** Per-segment `transport_strategy` feeds `TransportPlan` (delta vs full replay, prefix stability) before Phase 27 dispatch — aligns structural prompts with wire economics.
>
...
>
> **Estimated effort.** 8 days (1 PR, after Phase 19 lands streaming/tool-diff; may ship in same M3 window as 19).
...
4. Attach `SegmentPlan` to `ExecutionPlan` (extend metadata; do not fork second `ExecutionPlan` type — Phase 12 collapsed duplicates).
...
```
feat(planner): segment-aware planning policies [Phase 19.5]
