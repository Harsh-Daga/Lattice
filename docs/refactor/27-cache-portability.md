# Phase 27 — Cross-Provider Cache Portability, Cold-Start Warmer, KV-Cache Compatibility Analyzer

> **Footprint impact.** Zero new runtime deps. Portability manifests are signed using the same HMAC machinery [Phase 28](28-receipts.md) uses for receipts (which uses `pyjwt`, already an optional `[auth]` dep — if `[auth]` isn't installed, the portability manifest falls back to unsigned). CLI utilities ship in the base wheel.
>
> **Algorithm location.** New `src/lattice/cache/portability/` package, `src/lattice/cache/warmer.py`, `src/lattice/cache/analyzer.py`. All reuse Phase 20's tier and tenant infrastructure. No SDK involvement — the portability CLI talks to the local proxy's `/lattice/cache/{warm,analyze}` JSON endpoints.
>
> **External-service requirement.** None. No cloud, no external storage, no third-party manifest server. Manifests live in the same backend Phase 20 is using (memory / Redis / Postgres). The user-initiated provider switch is entirely local.
>
...
| [src/lattice/cache/ir_fingerprint.py](../../src/lattice/cache/ir_fingerprint.py) (from Phase 20) | Add `provider_invariant_fingerprint()` that excludes provider-specific metadata |
...
class PortableCacheManifest:
    ...
    ir_fingerprint: str                          # canonical IR hash (Phase 20)
    ...
    response_blob_hash: str                      # CAS pointer (Phase 20)

    # Signature
    signature: str                               # HMAC over the above; Phase 28 receipt-aligned key
```
...
When a response is cached (Phase 20), we also create or update the manifest:
...
Manifest signature uses the same HMAC key as the [Phase 28](28-receipts.md) receipts — single key rotation surface. Signature verifies integrity on load (prevents tampering); not required for correctness but required for cloud multi-tenant trust ([Phase 32](32-cloud-multitenant.md)).
...
4. Manifest signature is required before trusting cross-provider re-key lookups in cloud multi-tenant mode (Phase 32).
...
Phase 28 segment policies may assign higher cache weight to `history` / `instruction` segments. Phase 28 bandit tunes global weights — this phase supplies **measurement**, not routing.
