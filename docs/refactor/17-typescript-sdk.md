# Phase 17 — TypeScript SDK (`@lattice/sdk`) as a Thin Client

> **Footprint impact.** npm bundle ≤ 25 KB gzipped (edge entry); ≤ 60 KB gzipped (node entry). Zero runtime dependencies — `openai`, `@anthropic-ai/sdk`, `ai` are peer-deps only. The optional `@lattice/core-wasm` ([Phase 24](24-edge-wasm-core.md)) adds ≤ 200 KB gzipped when the user opts into in-process compression.
>
...
## 3. The two operating modes (mirrors the doctrine from [Phase 24](24-edge-wasm-core.md))
...
Three lines. The proxy handles compression, reverse-pass, cache, guardrails, everything. Streaming chunks arrive at the SDK **already decoded** (proxy did the work in [Phase 19](19-compression-intelligence.md)). The SDK has nothing to do.
...
[Phase 13](13-python-sdk-quality.md) typed the Python SDK. To keep TS and Python types in lockstep, both generate from a single OpenAPI spec.

`openapi/lattice-proxy.yaml` covers every endpoint:

- `POST /v1/chat/completions` (request + streaming response)
- `POST /v1/embeddings` ([Phase 20](20-non-chat-surfaces.md))
...
- `POST /lattice/cache/warm`, `POST /lattice/cache/analyze` ([Phase 22](22-cache-portability.md))
- `GET /lattice/receipts/{id}` ([Phase 23](23-receipts-bandit-profiles.md))
...
# Python TypedDicts (used by Phase 13 SDK)
...
Three callback lists. Same shape as Python ([Phase 13](13-python-sdk-quality.md) §4.6) — generated from the OpenAPI spec where possible. Pre-built helpers (`hooks/builtin.ts`): `logToConsole()`, `redactKeys(set)`, `attachRequestId()`, `emitOTel(tracer)` (when `@opentelemetry/api` is present).
