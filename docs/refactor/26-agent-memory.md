# Phase 26 — Agent Memory: Context GC, Summarization, Token Budgets, Inference-Aware Retry (lightweight)

> **Footprint impact.** **Zero new runtime deps in the default install.** Rule-based relevance scoring (recency × tool-call density × ANSI-token similarity) is the default. Optional embedding-based scoring reuses the **user's provider** embeddings endpoint from [Phase 20](17-hybrid-semantic-cache.md) — still no model download. Summarization optionally uses the **user's cheap model** (e.g. `gpt-4o-mini`, `claude-3-5-haiku`) — no additional vendor relationship, no download. The user already pays for these; we just route through them when summarization is needed.
>
> **Algorithm location.** New `src/lattice/agent/` package. The relevance scorer and budget allocator are pure functions; summarization is a request to the user's provider via the existing dispatcher (so it benefits from the cache, the bandit, and observability for free). Inference-aware retry composes with [Phase 21](18-native-guardrails.md)'s JSON repair and [Phase 27](22-compression-intelligence.md)'s structural repair.
>
...
>
> **Estimated effort.** 5 days (1 PR, ~+2000 LoC — smaller than prior because relevance scoring is rule-based by default; embeddings tier reuses Phase 20 infrastructure).
...
The prior plan defaulted to embedding-based relevance scoring using Phase 20's local SentenceTransformer (which we already replaced with user-provider embeddings in this rewrite). Adding embedding calls to every loop iteration was both costly (per-iteration provider call) and circular (Phase 20 now defaults to no-embedding).
...
Embedding-based scoring stays as a config option, reusing whatever embedding backend Phase 20 is configured with (default: user's provider; opt-in: local). No new infrastructure.
...
class EmbeddingRelevanceScorer:
    ...
    Reuses Phase 20's EmbeddingBackend — so by default it uses the user's provider
...
class Summarizer:
    ...
    and at least `min_tokens` long. Cached via Phase 20 cache by session +
...
class RetryStrategy(StrEnum):
    NONE = "none"
    STRUCTURAL_REPAIR = "structural_repair"    # Phase 21 / 19 — no LLM call
...
Composes with [Phase 21](18-native-guardrails.md) output validator and [Phase 27](22-compression-intelligence.md) `repair_v2`. The "minimal correction" prompt is a 1-line continuation (`"Continue from your last token."`) — cheaper than a full retry by ~ 90%.
...
5. Optional embedding-based scorer reuses Phase 20's embedding backend (defaults to user's provider — no model download); falls back to rule scorer if backend unhealthy.
...
| Per-user fine-tuned relevance models | **Cut from plan** — cloud + GPU + training infra. Bandit (Phase 28) covers a slice with zero infra. |
