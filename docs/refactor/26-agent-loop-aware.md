# Phase 26 — Agent-Loop-Aware Compression, Cursor Visualizer, Minimal Release
...
>
> **Algorithm location.** Step classifier in `src/lattice/agent/step_classifier.py`; uses [Phase 21](21-agent-memory.md)'s relevance scorer infrastructure. Per-step profiles wire into [Phase 23](23-receipts-bandit-profiles.md)'s profile system. Cursor extension is a thin TypeScript client that reads receipts ([Phase 23](23-receipts-bandit-profiles.md)) and OTel spans ([Phase 16](16-otel-genai.md)) — no algorithm code.
>
...
- npm package (already in CI from [Phase 17](17-typescript-sdk.md))
- Cursor extension as sideload `.vsix` (built in CI; downloadable from GitHub Releases)
- A simple `docker-compose.yml` example for self-hosted multi-user with Postgres ([Phase 25](25-cloud-multitenant.md))
...
tools/cursor-extension/
  ...
  src/
    extension.ts                                 # entry
    receiptsClient.ts                            # fetch from /lattice/receipts (Phase 23)
...
deploy/
  docker/
    ...
    docker-compose.example.yml                   # proxy + postgres example for Phase 25 multi-user
...
The override layers on top of the tenant's base profile (from [Phase 23](23-receipts-bandit-profiles.md)). Users can supply their own per-step profiles via config.
...
- `lattice.proxyUrl`
- `lattice.apiKey` (when proxy has [Phase 25](25-cloud-multitenant.md) auth enabled)
...
- Bandit reward attribution (Phase 23) is partitioned by step type so each profile self-tunes
...
| Footprint | `tests/integration/footprint/test_4gb_laptop.py` | unchanged from Phase 25 |
...
6. `npm install @lattice/sdk@2.0.0` works in a fresh Node 22 project; quickstart from [Phase 17](17-typescript-sdk.md) README runs without errors.
...
| Hosted lattice.cloud, SaaS, managed offering | Open-source self-hosted only ([Phase 25](25-cloud-multitenant.md) §6) |
