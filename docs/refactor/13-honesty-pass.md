...
|---|---|---|
| **Two `ExecutionPlan` types** | `rg -nl "class ExecutionPlan"` | One home in `ir/primitives.py`; `planner` re-exports; `session_plan.py` for session metadata only. |
| **Two `Candidate.score` formulas** | `rg -nl "def score|class CandidateScorer"` | Already named — one home. |
| **Per-adapter retry helpers** (~ 17 copies) | `rg -nl "for attempt in range" src/lattice/providers/adapters/` | Audit only here; resolved in [Phase 14](14-transport-layer-consolidation.md). Contract test `test_no_per_adapter_httpx_client.py` lands in Phase 14. |
| **Multiple `httpx.AsyncClient` instantiation sites** | `rg -nl "httpx.AsyncClient\(" src/lattice/` | Same — flagged here, fixed in [Phase 14](14-transport-layer-consolidation.md). |
...
| **Multiple chunk-buffer / streaming-decoder implementations** | `rg -nl "def feed.*chunk\|class.*Buffer"` | One in `pipeline/streaming/chunk_buffer.py`. Rust mirror in Phase 31; parity-tested. |
| **Multiple receipt / audit-log schemas** | `rg -nl "class.*Receipt\|class.*AuditLog"` | One in `audit/receipts.py` (lands in Phase 28). Phase 16 reserves the path. |
| **Multiple rate-limit parsers** (per-adapter `retry-after` parsing) | `rg -nl "retry-after\|ratelimit-reset"` in adapters | Flagged here, consolidated in [Phase 14](14-transport-layer-consolidation.md). |
...
Production evals and the architecture review surfaced **scattered validation** (MILV, guardrails, `ir/validation`, per-optimizer checks) without a single operator-facing story. Phase 13 adds a **facade only**:
...
| `src/lattice/pipeline/post_transform_guard.py` | Renamed from misleading `MILV`; still owns post-transform checks |
...
**Scoring honesty:** collapse `Candidate.score` / `CandidateScorer` to the utility formula in [`runtime.md`](../architecture/runtime.md#the-scoring-rule). Remove duplicate `_estimate_utility` bonus tables from call sites; Phase 28 bandit learns weights — Phase 16 only ensures **one formula home**.
...
| `src/lattice/pipeline/batch_accumulator.py` | `src/lattice/pipeline/request_coalescer.py` | `_flush_provider` stub deleted; class becomes the in-process coalescer it actually is. Real Batch API arrives in Phase 31, not here. |
...
Real OpenAI / Anthropic Batch API support is documented in [Phase 31](24-non-chat-surfaces.md) — leave a `# Real Batch API: see docs/refactor/24-non-chat-surfaces.md` comment in the file.
...
class ExecutionPlan:
    ...
    # Phase 16: fields previously on planner.execution_plan.ExecutionPlan
    tier: Tier | None = None
...
7. `docs/MIGRATION.md` updated with a "Phase 13" section listing every rename so downstream consumers know what to change.
...
| Topic | Phase |
|---|---|
| Real OpenAI/Anthropic Batch API integration | [Phase 24](24-non-chat-surfaces.md) |
| `_estimate_utility` → real expected-utility optimization | [Phase 29](29-bandit-routing.md) (bandit); inputs from [27](27-cache-portability.md), [14](14-transport-layer-consolidation.md) |
| Segment-aware planning | [Phase 23](23-segment-aware-planning.md) |
| LLMLingua-2 plug-in | [Phase 22](22-compression-intelligence.md) |
| TypeScript SDK | [Phase 20](20-typescript-sdk.md) |
