...
- [x] Move `runtime_contract` and `prefix_optimizer` onto immutable IR on the v2 path
- [x] Move `tool_filter` onto immutable IR on the v2 path
- [x] **Remove duplicate transform execution** — Legacy output tail (`reference_sub`, `tool_filter`, `output_cleanup`) was running twice in v2 pipeline. Removed from `build_v2_pipeline()` outer registration; PipelineV2 now handles all transforms from ExecutionPlan.
- [x] **Fix placeholder leakage guard** — `pipeline_v2` was not in `_placeholder_using_transforms`, causing the outer `CompressorPipeline` to roll back all `<ref_N>` substitutions produced by beam search. Added `pipeline_v2` to the exempt set.
- [x] **Convert remaining transforms to IRTransform** — 13 done (`runtime_contract`, `prefix_optimizer`, `tool_filter`, `reference_sub`, `message_dedup`, `format_conversion`, `rate_distortion`, `strategy_selector`, `cache_arbitrage`, `constraint_lifting`, `causal_chain`, `path_prefix`, `tool_projection`). Remaining legacy-only: content_profiler, output_cleanup.
- [x] **Wire IR-native transforms into the canonical v2 path** — `pipeline_v2._IR_NATIVE_CORE_TRANSFORMS` now explicitly lists `message_dedup`, `strategy_selector`, `cache_arbitrage`, `constraint_lifting`, `causal_chain`, `format_conversion`, `rate_distortion`, `reference_sub`. `UnifiedPlanner._TRANSFORM_ORDER` and `_TIER_ALLOWED` include these in the canonical plan.
