...
class TestRepresentationOptimizerLayer:
    ...
    def test_pipeline_runs_end_to_end_with_optimizers(self) -> None:
        ...
        async def run():
            ...
            # Schedule was generated by content_profiler
            assert "_lattice_schedule" in ctx.session_state
            # representation_optimizer ran
            assert any(
                "representation_optimizer" in name for name in ctx.transforms_applied
            ) or "representation_optimizer" in [t.name for t in pipeline.transforms]
