...
class TestIRBuilder:
    ...
    def test_ir_marks_error_messages_protected(self) -> None:
        req = _make_request(
            [
                {
                    "role": "user",
                    "content": "Build failed with 20 Module not found errors and 20 Syntax errors.",
                },
            ]
        )
        ir = build_ir(req)
    ...
    def test_ir_classifies_constraints(self) -> None:
        ...
        ir = build_ir(req)
        assert any(
            s.type == SectionType.CONSTRAINTS or SectionType.INSTRUCTION for s in ir.sections
        )
    ...
    def test_normalize_json_detects_constant_fields(self) -> None:
        req = _make_request(
            [
                {
                    "role": "tool",
                    "content": '[{"status":"ok","latency":100},{"status":"ok","latency":101},{"status":"ok","latency":102}]',
                },
            ]
        )
        ir = build_ir(req)
    ...
    def test_compiler_produces_valid_ir(self) -> None:
        ...
        req = _make_request(
            [
                ...
                {
                    "role": "user",
                    "content": "Analyze the root cause: service A failed, causing B timeout.",
                },
            ]
        )
