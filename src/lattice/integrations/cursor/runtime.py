"""Cursor runtime helpers for LATTICE lace."""

from __future__ import annotations

from dataclasses import dataclass

from lattice.integrations.claude.runtime import proxy_base_url as claude_proxy_base_url
from lattice.integrations.codex.runtime import proxy_base_url as codex_proxy_base_url
...
def build_launch_env(port: int, backend: str = "openai") -> dict[str, str]:
    """Build environment variables for Cursor (used if launching via CLI)."""
...
def render_setup_lines(port: int) -> list[str]:
    ...
    targets = build_proxy_targets(port)
    return [
        "  LATTICE proxy is running. Configure Cursor:",
        "",
        "  For OpenAI models:",
